...
class RGBOnlyDETR3DHead(BaseModule):
    ...
    def _decode_boxes(self, box_predictions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        centers = self.anchor_centers.unsqueeze(0) + center_offsets
    ...
    def forward(self, mlvl_feats: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        normalized_anchors = ((self.anchor_centers - self.center_min) /
                              (self.center_max - self.center_min)).clamp(0., 1.)
        query_positions = self.anchor_position_embedding(normalized_anchors)
        query = (self.query_embedding.weight + query_positions).unsqueeze(0)
        query = query.expand(memory.size(0), -1, -1)
        ...
        for level, decoder in enumerate(self.decoder):
            ...
            boxes, center_offsets = self._decode_boxes(self.box_branches[level](query))
