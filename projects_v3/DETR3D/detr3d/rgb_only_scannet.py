...
class RGBOnlyScanNetDataset(Det3DDataset):
    ...
    def parse_data_info(self, info: dict) -> dict:
        ...
        if not point_info or 'lidar_path' not in point_info:
            raise ValueError('Missing lidar_points required for point-anchor initialization.')
        point_path = point_info['lidar_path']
        if not osp.dirname(point_path):
            point_path = osp.join('points', point_path)
        info['anchor_point_path'] = osp.join(self.data_root, point_path)
        info['anchor_axis_align_matrix'] = np.asarray(
            info.get('axis_align_matrix', np.eye(4)), dtype=np.float32)
