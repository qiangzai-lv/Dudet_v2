...
class RGBOnlyDETR3DHead(BaseModule):
    ...
    def _decode_boxes(self, normalized_boxes: torch.Tensor) -> torch.Tensor:
        centers = self.center_min + normalized_boxes[..., :3] * (self.center_max - self.center_min)
        sizes = self.size_min + normalized_boxes[..., 3:] * (self.size_max - self.size_min)
    ...
    def forward(self, mlvl_feats: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
        memory = self._build_memory(mlvl_feats)
        query = self.query_embedding.weight.unsqueeze(0).expand(memory.size(0), -1, -1)
        ...
        for level, decoder in enumerate(self.decoder):
            ...
            all_bbox_preds.append(self._decode_boxes(self.box_branches[level](query).sigmoid()))
        return {'all_cls_scores': torch.stack(all_cls_scores),
